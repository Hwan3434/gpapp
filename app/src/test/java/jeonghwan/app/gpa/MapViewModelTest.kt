...
class MapViewModelTest {
    ...
    // 더미 TombEntity 생성
    val dummyTombEntity = TombEntity(
        key = 1,
        name = "Doe Tomb",
        location = GpGeoPoint(
            latitude = 37.5665,
    ...
    // 더미 PersonEntity 생성
    val dummyPersonEntity = PersonEntity(
        key = 1,
        spouse = null,
        generator = 1,
        gender = true, // 남성
    ...
    @Test
    fun `loadTombData should update uiState on success`() = runTest {
    ...
    @Test
    fun `loadTombData should update errorMessage on failure`() = runTest {
