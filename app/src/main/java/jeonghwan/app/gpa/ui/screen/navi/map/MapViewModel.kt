...
data class TombDataModel(
    ...
    fun getCaption(): String {
        assert(isFirstPerson())
        val p = persons.first()
        return if (p.gender) p.family else p.name
    }

    fun getColor(): Color {
        assert(isFirstPerson())
        return if (persons.first().gender) Color.Magenta else Color.Blue
    }
}
...
class MapViewModel @Inject constructor(
    ...
    fun toggleTombPopupWindowVisibility(tombKey: Int) {
        _uiState.value = _uiState.value.copy(
           tombs = _uiState.value.tombs.map { tombData ->
